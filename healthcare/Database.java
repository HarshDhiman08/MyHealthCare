...
public class Database extends SQLiteOpenHelper {
    ...
    public ArrayList getCartData(String username, String otype){
        ...
        str[0]= username;
        str[1]= otype;
        @SuppressLint("Recycle") Cursor c= db.rawQuery("select * from cart where username = ? and otype = ?",str);
        if (c.moveToFirst()){
            do {
    ...
    public ArrayList getOrderData(String username){
        ...
        str[0]= username;
        @SuppressLint("Recycle") Cursor c= db.rawQuery("select * from orderplace where username = ?",str);
        if (c.moveToFirst()){
            do {
    ...
    public int checkAppintmentExists(String username,String fullname, String address ,String contact ,String date, String time){
        ...
        str[5]= time;
        SQLiteDatabase db = getReadableDatabase();
        @SuppressLint("Recycle") Cursor c = db.rawQuery("select * from orderplace where username = ? and fullname=? and address = ? and contact = ? and date = ? and time = ?",str);
        if (c.moveToFirst()){
            result =1;
        }
        db.close();
